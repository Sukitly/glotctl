...
<Info>**Severity: Error** — causes exit code 1 and fails CI builds by default.</Info>
...
Untranslated values are reported as **errors** by default because real untranslated UI text reaches users as a product issue.
...
For existing projects, `glot baseline --rules untranslated --apply` can insert suppression comments for current issues so new untranslated values are still caught.
