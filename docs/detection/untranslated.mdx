...
<Info>**Severity: Error when used in source; Warning when no usage is found.** Used untranslated values cause exit code 1 by default.</Info>
...
error: "common.submit"  [untranslated]
  ...
  = used: ./src/app.tsx:10:18

error: "common.cancel"  [untranslated]
  --> ./messages/en.json:4:1
  = note: ("Cancel") identical in: zh
  = used: ./src/app.tsx:11:18
...
Untranslated values are reported as **errors** by default when the key is used in source because real untranslated UI text reaches users as a product issue.

If glot cannot find any source usage for the key, the issue is reported as a **warning** by default. That means the catalog value still looks untranslated, but glot has not proven that it reaches users. You can still make catalog-only untranslated values fail CI with:
