...
Use multiple comments for complex patterns:

```tsx
// glot-message-keys "user.*.name", "user.*.email"
// glot-message-keys "admin.*.name", "admin.*.email"
function renderProfile(role, userId) {
  return (
    <div>
      <span>{t(`${role}.${userId}.name`)}</span>
      <span>{t(`${role}.${userId}.email`)}</span>
    </div>
  );
}
```
...
Place annotations **before** the dynamic key usage:

```tsx
// ✓ Correct - before the usage
// glot-message-keys "status.*"
return <span>{t(`status.${code}`)}</span>;

// ✗ Wrong - after the usage
return <span>{t(`status.${code}`)}</span>;
// glot-message-keys "status.*"
```
