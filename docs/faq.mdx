...
**No.** Glot is still focused on React-style source analysis. It does not support Vue, Angular, or Svelte projects.

</Accordion>

<Accordion title="Does glot support Astro?">

**Partially.** Glot can analyze translation key usage inside supported `.astro` files:

- frontmatter static and template-literal `t(...)` calls
- template expressions like `{t("...")}`
- template attributes like `title={t("...")}`
- template dynamic-key declarations via `<!-- glot-message-keys "..." -->`

**Current limitation:** hardcoded text detection does **not** run on `.astro` files yet.
...
- `glot fix --apply` — inserts `glot-message-keys` comments (`//`, `{/* */}`, or `<!-- -->` depending on context)
...
1. **Single line**: Add `// glot-disable-next-line` (or `{/* glot-disable-next-line */}` in JSX) above the line.
   ...
   Astro note: hardcoded text suppression is not needed today because the hardcoded-text rule does not run on `.astro` files yet.
...
In Astro templates, use HTML comments instead:
...
**Default errors** (exit code 1): hardcoded text, missing keys, replica-lag, used untranslated values, type mismatches.

**Default warnings** (exit code 0): orphan keys, unused keys, unresolved keys, untranslated values with no source usage.

Use `glot check --error-on-warnings` in CI if any warning should fail the build. You can also change rule severities with the `severities` configuration option.
...
1. Run `npx glot baseline --apply` to suppress all existing hardcoded text issues
