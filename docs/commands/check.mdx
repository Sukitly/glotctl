...
| -------- | --------- | ----------------------------------------------------------------- |
| Error    | 1         | Hardcoded text, missing keys, replica lag, untranslated values, type mismatches |
| Warning  | 0 by default, 1 with `--error-on-warnings` | Orphan keys, unused keys, unresolved keys, or rules downgraded via config |
...
| `untranslated` | Error | Value identical to primary locale | [Untranslated](/detection/untranslated) |
...
**Untranslated values** → Translate the non-primary locale value, or suppress intentional same-language values with `glot-disable-next-line untranslated`. See [Untranslated](/detection/untranslated).
