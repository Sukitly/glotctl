...
<Note>
  `baseline --rules untranslated` inserts source comments only for untranslated
  values that have source usages. Untranslated catalog values with no usage
  cannot be suppressed with source comments; translate them, remove unused keys,
  or configure `severities.untranslated` for a project-wide policy.
</Note>
